...
export function useChatFinalize({
  ...
  /**
   * 再入ガード。ボタンの disabled は finalizing state 依存なので、遷移完了までの間に
   * state を戻すと連打で onFinalize が再実行され、先頭の AI 抽出（extractFieldsFromChat /
   * /api/minutes/chat/extract-fields）が二重に走って API コストが無駄になる。
   * state と違い ref は同期的に立つため、await を挟む前に確実に二重起動を止められる。
   * 画面遷移する経路では立てたままにし、その場に留まるエラー経路でのみ戻して再試行を許す。
   */
  ...
  async function onFinalize() {
    ...
    setFinalizing(true)
    setErrorMsg(null)
    // 画面遷移まで到達したかどうか。到達した経路ではガードを開放しない（遷移待ちの間に
    // 押し直されると AI 抽出が二重に走る）。その場に留まる経路でのみ finally で戻して
    // 再試行を許す。
    let navigated = false
    ...
    try {
      ...
      // result は抽出の try 内で代入され、meetingDate 確定（下方）はその外。extractFailed 時は
      ...
      try {
        ...
        clearSnapshot()
        router.push(`/minutes/${created.id}/adjust`)
        navigated = true
      } catch (e) {
        // 失敗時は遷移しない（ChatView に留まる）。finalizing の開放は finally が行う。
      ...
      // 遷移前の sessionStorage 書き込み / form-cache 書き込みなどが throw した場合の受け皿。
      // ここで拾わないと finalizing が true のまま残り、ボタンが「準備中…」で固まる。
      setErrorMsg(
        `保存に失敗しました: ${humanizeErrorCode(e instanceof Error ? e.message : null).message}`,
      )
    } finally {
      // 遷移した経路では戻さない（連打による AI 抽出の二重実行を防ぐガードを維持する）。
      if (!navigated) {
