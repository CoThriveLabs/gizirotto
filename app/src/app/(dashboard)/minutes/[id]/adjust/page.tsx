...
export default async function MinutesAdjustPage({
  ...
  const initialOverrides = parseFieldOverrides(minutes.bbox_overrides)

  // builtin テンプレ × 旧議事録（bbox_overrides 未焼き込み）でも、サーバ側で bbox JSON を
  //   読み込み bboxFallback に合流させる。これにより seed.sql で bbox を持たない field が
  //   AdjustView で「描画されない（fields から脱落）」問題を構造的に解消する。
  //
  //   合流ポリシー: initialOverrides（DB 由来・ユーザー編集差分）を最優先。
  //     bbox JSON 側は **欠落 key の補完のみ**。ユーザーが手で動かした座標は絶対に
  //     上書きしない（mistake.md「ユーザー未指示の機能撤去禁止」遵守）。
  //
  ...
  > | null = null
  if (template && (template as { family_id?: string | null }).family_id === null) {
    const slug = resolveBuiltinBboxSlugFromProcessedPath(
      (template as { processed_path?: string | null }).processed_path ?? null,
    )
    if (slug) {
      try {
  ...
  // builtin テンプレは templates.fields に bbox が無い（seed 由来）。
  //   createMinute で minute.bbox_overrides に初期 bbox が焼き込まれているはずなので、
  //   それを fields の bbox 欠落 fallback として渡し、AdjustView 起動時の「fields 0 個」
  //   問題を解消する。user テンプレは bbox が DB に揃っているため
  //   fallback は使われず副作用ゼロ。
  const bboxFallbackForFields: Record<string, { x: number; y: number; w: number; h: number }> = {}
  // 旧議事録救済として JSON 側を先に merge し、その後 initialOverrides で上書き
  //   （ユーザー編集差分を最優先）。
  ...
  //   - 既存 minute（new_fields = []）は tplPdfFields をそのまま返す（不変・後方互換）。
  const newFieldsParsed = parseNewFields(minutes.new_fields)
  ...
  const initialValues = parseContentValues(minutes.content_json, fields)
...
/**
 * 🔴 D3 案 B FB（2026-06-09 実機判明真因 ①②）: テンプレ DB の実 PdfField[] を抽出する。
 *
 ...
 * font_size_min を完全同型化** するための入口。旧 AdjustView.toPdfField の padding ハードコード
 * （{left:2, top:2, right:2, bottom:2}）が canvas 側 maxW を広げて wrap 位置を PDF より遅らせる
 * 真因だったので、ここで実テンプレ値を取り出して props 経由で渡す。
 *
 ...
 */
function extractPdfFields(
  raw: unknown,
  fields: TemplateFieldDef[],
): PdfField[] {
  // raw を { name -> 生 jsonb } マップに正規化（page.tsx 内 extractFieldDefs と同じ走査）。
  const fieldsArr: unknown = Array.isArray(raw)
    ? raw
 ...
 */
function extractFixedTextSizesPt(raw: unknown): number[] {
